  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_scf == _TRUE_) {
        ...
        ppv->y[ppv->index_pt_delta0_scf] =
          ppw->pv->y[ppw->pv->index_pt_delta0_scf];

        ppv->y[ppv->index_pt_delta1_scf] =
          ppw->pv->y[ppw->pv->index_pt_delta1_scf];
      }
...
int perturb_einstein(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_delta_m == _TRUE_) {
      ...
      // 1307.1459v1,omega,v3. It came from a confusion between (1+w_total)
...
int perturb_sources(
  ...
  double delta_g, delta_scf, rho_plus_p_theta_scf;
...
int perturb_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    /** ---> Equations of motion for the density contrasts */
        dy[pv->index_pt_delta0_scf] = 0.;/*-a_prime_over_a*((3.*sin_scf(pba,theta_phi_scf)+omega_scf*(1.-cos_scf(pba,theta_phi_scf)))*delta1_scf
